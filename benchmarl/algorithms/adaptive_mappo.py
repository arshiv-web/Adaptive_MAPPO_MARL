...
class AdaptiveAlphaNetwork(nn.Module):
    def __init__(self, hidden_dim=16):
        ...
        # We maintain 6 features:
        # v_g_norm, v_l_norm, f_disagree, f_team_var, f_agent_dev, f_ratio
        ...
        self.alpha_mean = torch.tensor(0.0)
        self.alpha_std = torch.tensor(0.0)
        self.alpha_min = torch.tensor(0.0)
        self.alpha_max = torch.tensor(0.0)

    def forward(self, v_global: torch.Tensor, v_local: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        # 1. Base Normalization (Anchoring both to the global scale)
        v_mean = v_global.mean(dim=(-2, -1), keepdim=True)
        
        v_std = v_global.std(dim=(-2, -1), keepdim=True, unbiased=False) + 1e-5

        v_g_norm = (v_global - v_mean) / v_std
        v_l_norm = (v_local - v_mean) / v_std
        
        # 2. Derived Features (Computed on normalized space)
        f_disagree = torch.abs(v_g_norm - v_l_norm)
        
        if v_local.shape[-2] > 1:
            team_mean = v_l_norm.mean(dim=-2, keepdim=True)
            f_team_var = v_l_norm.var(dim=-2, keepdim=True).expand_as(v_l_norm)
            f_agent_dev = v_l_norm - team_mean
        else:
            f_team_var = torch.zeros_like(v_l_norm)
            f_agent_dev = torch.zeros_like(v_l_norm)
            
        # 3. Log-Ratio (Computed on raw space to avoid negative log issues)
        eps = 1e-5
        f_ratio = torch.log((torch.abs(v_global) + eps) / (torch.abs(v_local) + eps))
        
        # 4. Concatenate and pass through MLP
        features = torch.cat([
            v_g_norm, 
            v_l_norm, 
            f_disagree, 
        ...
        # 5. Output Alpha and Mix (Mix MUST use raw values for the PPO target!)
        alpha = torch.sigmoid(logits)
        with torch.no_grad():
            self.alpha_mean = alpha.mean().detach()
            self.alpha_std = alpha.std().detach()
            self.alpha_min = alpha.min().detach()
            self.alpha_max = alpha.max().detach()
